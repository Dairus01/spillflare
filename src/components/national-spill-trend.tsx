...
export function NationalSpillTrend({ data, scope = "Nigerian" }: { data: YearlySpillCount[]; scope?: string }) {
  return (
    <div className="national-trend-chart" role="img" aria-label={`Bar chart of ${scope} oil spill records by year from ${data[0]?.year ?? "the first supplied year"} to ${data.at(-1)?.year ?? "the latest supplied year"}`}>
