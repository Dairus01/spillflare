...
export default async function OilSpillsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <section className="page-hero oil-spill-hero"><div className="container"><span className="eyebrow">NOSDRA oil spill records · Nigeria</span><h1>Nigeria Oil Spill Records and Tracker</h1><p>Search public Nigerian oil spill records by location, state, operator and year. Examine reported incidents on the map, inspect the fields supplied for each record and follow stable record pages back to their source context.</p><div className="button-row"><a className="button" href="#incident-explorer"><Search size={16} />Search the records</a><Link className="button secondary" href="/oil-spills/archive"><Database size={16} />Browse record archive</Link><Link className="button secondary" href="/oil-spills/analytics"><BarChart3 size={16} />Explore detailed analytics</Link><Link className="button secondary" href="/oil-spills/causes"><BarChart3 size={16} />Explore reported causes</Link><Link className="button secondary" href="/oil-spills/niger-delta"><BarChart3 size={16} />Explore Niger Delta records</Link></div></div></section>
