...
const title = "Oil Spills in Nigeria: Tracker, Map & Data | SpillFlare";
const openGraphTitle = "Oil Spills in Nigeria: Tracker, Map & Data";
const description = "Explore oil spills in Nigeria with SpillFlare. Search public records, track recent incidents, compare states and years, and explore national oil spill data.";
...
export async function generateMetadata({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: openGraphTitle, description, type: "website", url: "/oil-spills" },
...
export default async function OilSpillsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <section className="page-hero oil-spill-hero"><div className="container"><span className="eyebrow">NOSDRA oil spill records · Nigeria</span><h1>Oil Spills in Nigeria</h1><p>Explore reported oil spills in Nigeria through SpillFlare&apos;s searchable public records. Track recent incidents, search by location, state, operator and year, and examine historical records using the national oil spill map, explorer and archive.</p><div className="button-row"><a className="button" href="#incident-explorer"><Search size={16} />Search Nigeria oil spill records</a><Link className="button secondary" href="/oil-spills/archive"><Database size={16} />Browse Nigeria oil spill records</Link><Link className="button secondary" href="/oil-spills/analytics"><BarChart3 size={16} />View Nigeria oil spill analytics</Link><Link className="button secondary" href="/oil-spills/causes"><BarChart3 size={16} />Explore reported oil spill causes</Link><Link className="button secondary" href="/oil-spills/niger-delta"><BarChart3 size={16} />Explore Niger Delta oil spill records</Link></div></div></section>
    ...
    <div className="wide-container oil-spill-page">
      ...
      <section id="incident-explorer" className="oil-spill-explorer" aria-labelledby="explorer-heading">
        <SectionHeading eyebrow="Search and map the source records" title="Nigeria oil spill map and explorer" body="Filter the incident database without losing records that have missing quantities or unusable coordinates." />
        ...
        <div className="split" style={{ marginTop: 18 }}><NigeriaMap points={points} height={610} center={[6.2, 5.8]} zoom={7} /><div className="panel"><div className="panel-head"><h2 id="explorer-heading">Matching incidents</h2><span className="mono">Page {page}</span></div><div className="record-list">{paged.map((row) => <Link className="record-row" key={row.id} href={spillPath(row.id)}><time>{formatDate(row.incidentdate, { day: "2-digit", month: "short", year: "2-digit" })}</time><div><strong>{row.incidentnumber ?? row.id} · {row.company ?? "Company not supplied"}</strong><p>{row.sitelocationname ?? "Location not supplied"}</p><p>{codedLabel("contaminant", row.contaminant)} · {row.lga ?? "LGA not supplied"}, {stateCodes[row.statesaffected ?? ""] ?? row.statesaffected ?? "State not supplied"}</p></div><span className="status">{row.status ?? "Recorded"}</span></Link>)}</div><div className="panel-head"><Link className="button ghost" aria-disabled={page === 1} href={`?${queryString}&page=${Math.max(1, page - 1)}`}>Previous</Link>{page * pageSize < filtered.length ? <Link className="button ghost" href={`?${queryString}&page=${page + 1}`}>Next</Link> : <span className="button ghost disabled">Next</span>}</div></div></div>
      </section>

      <section className="oil-spill-authority" aria-labelledby="national-trend-heading"><SectionHeading eyebrow="National record overview" title="Oil spills in Nigeria by year" body="Counts use only trustworthy incident dates. Reporting practices and source coverage can affect comparisons between years." action={{ label: "View Nigeria oil spill analytics", href: "/oil-spills/analytics" }} /><div className="trend-layout"><div className="analytics-card"><div className="analytics-card-head"><h2 id="national-trend-heading">Dated incident records</h2><p>{coverage} · malformed and internally inconsistent dates excluded</p></div><NationalSpillTrend data={overview.yearly} /></div><div className="panel"><div className="panel-head"><h2>Latest annual counts</h2></div><div className="compact-ranking">{latestYears.map((item) => <div key={item.year}><Link href={`/oil-spills/archive/year/${item.year}`}>Oil spills in {item.year}</Link><strong>{formatNumber(item.count)}</strong></div>)}</div></div></div></section>

      <section className="oil-spill-authority"><div className="ranking-grid"><div><SectionHeading eyebrow="Geographic coverage" title="Oil spill records in Nigeria by state" body="States with the most records in the current SpillFlare dataset—not a ranking of pollution or environmental harm." action={{ label: "Explore all state records", href: "/places" }} /><ol className="authority-ranking">{topStates.map((item, index) => <li key={item.name}><span>{index + 1}</span><Link href={`/places/states/${slugify(item.name)}`}>Oil spills in {item.name} State</Link><strong>{formatNumber(item.count)} records</strong></li>)}</ol></div><div><SectionHeading eyebrow="Source operator field" title="Operators named most often in the records" body="Company names are preserved as supplied. Frequency does not establish causation, fault or legal liability." /><ol className="authority-ranking">{topOperators.map((item, index) => <li key={item.name}><span>{index + 1}</span><Link href={`/oil-spills?year=&company=${encodeURIComponent(item.name)}`}>{item.name} oil spill records</Link><strong>{formatNumber(item.count)} records</strong></li>)}</ol></div></div></section>

      <section className="oil-spill-authority"><SectionHeading eyebrow="Latest trustworthy incident dates" title="Recent oil spill records in Nigeria" body="These are the latest valid incident dates in the source snapshot, ordered by date and stable record ID." /><div className="panel record-list">{overview.recentRecords.map((row) => <Link className="record-row" key={row.id} href={spillPath(row.id)}><time>{formatDate(row.incidentdate, { day: "2-digit", month: "short", year: "numeric" })}</time><div><strong>{row.sitelocationname ?? row.lga ?? "Location not supplied"}</strong><p>{row.company ?? "Company not supplied"} · {stateCodes[row.statesaffected ?? ""] ?? row.statesaffected ?? "State not supplied"}</p><p>Incident {row.incidentnumber ?? row.id}</p></div><span className="status">Record</span></Link>)}</div></section>
